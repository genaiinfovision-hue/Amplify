            ...
            <div className="flex flex-col">
              {activity.length === 0 ? (
                <p className="py-2 text-sm text-gray-500">
                  No recent activity yet. Events from the database activity log will appear here.
                </p>
              ) : (
                activity.map((a, i) => (
                  <div
                    key={`${a.who}-${a.time}-${i}`}
                    className={`py-2.5 ${i < activity.length - 1 ? 'border-b border-gray-100' : ''}`}
                  >
                    <div className="text-sm text-gray-600">
                      <span className="font-semibold text-gray-900">{a.who}</span> {a.action}{' '}
